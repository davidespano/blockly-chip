...
Chip.init = function() {
  ...
  Chip.reset(true);
  BlocklyGames.workspace.addChangeListener(function() {Chip.updateCapacity()});
...
Chip.initInterpreter = function(interpreter, scope) {
  ...
  interpreter.setProperty(scope, 'turnDown',
      interpreter.createNativeFunction(wrapper));
  wrapper = function(id) {
    return interpreter.createPrimitive(Chip.isPath(1, id.toString()));
  };
  interpreter.setProperty(scope, 'isPathRight',
      interpreter.createNativeFunction(wrapper));
  wrapper = function(id) {
    return interpreter.createPrimitive(Chip.isPath(0, id.toString()));
  };
  interpreter.setProperty(scope, 'isPathUp',
      interpreter.createNativeFunction(wrapper));
  wrapper = function(id) {
    return interpreter.createPrimitive(Chip.isPath(3, id.toString()));
  };
  interpreter.setProperty(scope, 'isPathLeft',
      interpreter.createNativeFunction(wrapper));
  wrapper = function(id) {
    return interpreter.createPrimitive(Chip.isPath(2, id.toString()));
  };
...
Chip.animate = function() {
  ...
  switch (action[0]) {
    ...
    case 'west':
      ...
      break;
    case 'look_north':
      Chip.scheduleLook(Chip.DirectionType.NORTH);
      break;
    case 'look_east':
      Chip.scheduleLook(Chip.DirectionType.EAST);
      break;
    case 'look_south':
      Chip.scheduleLook(Chip.DirectionType.SOUTH);
      break;
    case 'look_west':
      Chip.scheduleLook(Chip.DirectionType.WEST);
      break;
    case 'fail_forward':
